...
CLASS LHC_SHOP IMPLEMENTATION.
  ...
  READ ENTITIES OF ZLG_EXTR_ShopTP_LGL IN LOCAL MODE
    ENTITY Shop
  ...
  SELECT MAX( ORDER_ID ) FROM ZLGEXTASHOPLGL INTO @DATA(max_object_id).
  "Add support for draft if used in modify
  "SELECT SINGLE FROM FROM ZLGEXTSHO00D_LGL FIELDS MAX( OrderID ) INTO @DATA(max_orderid_draft). "draft table
  "if max_orderid_draft > max_object_id
  " max_object_id = max_orderid_draft.
  "ENDIF.
  MODIFY ENTITIES OF ZLG_EXTR_ShopTP_LGL IN LOCAL MODE
    ENTITY Shop
